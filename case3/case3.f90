    program case3
      implicit none
      integer i,j,m,step,step_n,total_steps,total_nodes,n_near,n_correct,i_correct,count
      real*8, allocatable :: x(:),h(:,:),u(:,:),w(:,:),h_x(:,:),u_x(:,:),uh_x(:,:),x_c(:),q(:),q_x(:),zeta(:,:),zeta_x(:,:)
      real*8, allocatable :: weighting(:,:),lambda(:,:,:),zb(:),zb_x(:),u_bc(:),h_bc(:),output(:, :),flat(:, :)
      ...
      !allocate(output(1:total_nodes, 1:3))
      allocate(flat(1:501, 1:1255))
      ...
      call MLS(total_nodes,x,m,n_near,weighting,lambda,n_loc,loc_index,h(:,step),u(:,step),w(:,step) &
              ,h_x(:,step),u_x(:,step),uh_x(:,step),zb(:),zb_x(:),zeta(:,step),zeta_x(:,step))   
      ...
      hb=(h_bc(0)+h_bc(1))/2
      ub=(u_bc(0)+u_bc(1))/2
      !write(*,'(a5,a5)') 'step=','  1/2'
      call predict(total_nodes,step,dt,g,h,u,h_x,u_x,uh_x,zeta,zeta_x,zb)
	  ...
	  do i_correct=1,n_correct
        ...
        call correct(total_nodes,step,step_n,dt,g,h,u,h_x,u_x,uh_x,q,q_x,n_correct,i_correct,zeta,zeta_x,zb_x)
	    call NF_BC(total_nodes,step,u,kin)
	    call WM_BC(total_nodes,step,h,u,kin,hb,ub)
        call MLS(total_nodes,x,m,n_near,weighting,lambda,n_loc,loc_index,h(:,step),u(:,step),w(:,step) &
                ,h_x(:,step),u_x(:,step),uh_x(:,step),zb(:),zb_x(:),zeta(:,step),zeta_x(:,step))     
      ...
      do j=1,total_nodes
        h(j,step-1)=h(j,step)
        zeta(j,step-1)=zeta(j,step)
        u(j,step-1)=u(j,step)
		w(j,step-1)=w(j,step)
        h_x(j,step-1)=h_x(j,step)
        zeta_x(j,step-1)=zeta_x(j,step)
      ...
      do step_n=2,total_steps 
		...
		do i_correct=1,n_correct
          ...
          ! data collection to flat matrix
	!          if(step_n.ge.2500 .and. step_n.le.3000)then
	!            print *, step_n
	!            do i = 1,251
	!               print *, 'test'
	!               print *, u(i + 249,step_n)
	!             flat(step_n - 2499, i_correct + (n_correct * (i - 1)))  = u(i + 249,step_n)
	!            end do
	!          end if
        ...
        do j=1,total_nodes
          ...
          w(j,1)=w(j,2)
          h_x(j,1)=h_x(j,2)
          zeta_x(j,1)=zeta_x(j,2)
          u_x(j,1)=u_x(j,2)
          uh_x(j,1)=uh_x(j,2)
        end do
      ! space for step output data
      ! if(step_n.eq.5)then
      ! open(4,file='C:\Users\laserlab04\Desktop\case3_zeta.txt')
      ! do i = 1,total_nodes
        ! write(4,'(11f16.5)')x(i),zeta(i,step)
      ! end do
      ! close(4)
      ! end if
    ...
    ! output flat matrix
	!    open(4,file='C:\Users\laserlab04\Desktop\flatMatirx_forConvergence.txt')
	!      do i = 1,5000
	!        write(4,'(11f16.5)')(flat(i, j), j = 1, 5805)
	!      end do
	!    close(4)
...
!      p(4)=x**3
    ...
    subroutine MLS(N,x,m,n_near,wt,lambda,n_loc,loc_index,h,u,w,h_x,u_x,uh_x,zb,zb_x,zeta,zeta_x)
      implicit none
      integer i,j,k,l,m,N,n_near,n_loc(1:N),loc_index(1:N,1:n_near*5)
      real*8 x(1:N),h(1:N),u(1:N),w(1:N),new_zeta(1:N),new_u(1:N),smooth_w(1:N),aa & ! add new_zeta
            ,h_x(1:N),u_x(1:N),uh_x(1:N),zb(1:N),zb_x(1:N),lambda(1:N,1:m,1:n_near*5),wt(1:N,1:n_near*5),zeta(1:N),zeta_x(1:N) ! add  zb, zb_x, zeta, zeta_x
  ...
  !substitude
    ...
    subroutine predict(total_nodes,step,dt,g,h,u,h_x,u_x,uh_x,zeta,zeta_x,zb)      ! add zeta zeta_x zb
      implicit none
      integer j,total_nodes,step
      real*8 h(1:total_nodes,0:2),u(1:total_nodes,0:2),zeta(1:total_nodes,0:2)& ! add zeta
            ,h_x(1:total_nodes,0:2),u_x(1:total_nodes,0:2),uh_x(1:total_nodes,0:2),zeta_x(1:total_nodes,0:2),zb(1:total_nodes)&! add zeta_x && zb
            ,dt,g
      do j=1,total_nodes
!        if(step .eq. 1) then
          zeta(j,step)=zeta(j,step-1)-dt*uh_x(j,step-1)                  ! substitude h for zeta
		  h(j,step)=zeta(j,step)-zb(j)                                   ! calculate h
          u(j,step)=u(j,step-1)-dt*(g*zeta_x(j,step-1)+u(j,step-1)*u_x(j,step-1)) ! substitude h_x for zeta_x
!        else
!		  h(j,step)=h(j,step-1)-dt*(1.5d0*uh_x(j,step-1)-.5d0*uh_x(j,step-2))
!		  u(j,step)=u(j,step-1)-dt*(1.5d0*(g*h_x(j,step-1)+u(j,step-1)*u_x(j,step-1)) &
!		                            -.5d0*(g*h_x(j,step-2)+u(j,step-2)*u_x(j,step-2)))
!		end if
      end do
    end
    ...
    subroutine correct(total_nodes,step,step_n,dt,g,h,u,h_x,u_x,uh_x,q,q_x,n_correct,i_correct,zeta,zeta_x,zb_x)
      implicit none
      integer j,total_nodes,step,step_n,n_correct,i_correct
      real*8 h(1:total_nodes,0:2),u(1:total_nodes,0:2),q(1:total_nodes),q_x(1:total_nodes),zeta(1:total_nodes,0:2)&                ! add zeta
            ,h_x(1:total_nodes,0:2),u_x(1:total_nodes,0:2),uh_x(1:total_nodes,0:2),zeta_x(1:total_nodes,0:2),zb_x(1:total_nodes)&  ! add zeta_x && zb_x
            ,dt,g,s1,s2,s3,s4,s5,up,down,rr,constant
      character filename,str1,str2
      ...
      do j=1,total_nodes
        ...
        !s3=.5d0 * q_x(j) * (1-cos(3.14159265358979*rr/2)) * 0.5 ! another ramp function
        up = zeta_x(j,step-1) + zeta_x(j,step) + 2*zb_x(j)
        down = 1000*((h(j,step-1) + h(j,step)))
		s4=.5d0*q(j)*(up)/(down)*(1-abs(cos(3.14159265358979*rr/2))**2.d0)    !(1-abs(cos(3.14159265358979*rr/2))**2.d0) generally correct
        !s4=.5d0*q(j)*(up)/(down)*(1-cos(3.14159265358979*rr/2)) * 0.5        so it can increase numerical stability
		u(j,step)=u(j,step-1)-dt*(s1+s2+s3+s4)

      end do

!      if(step_n .ge. 2500 .and. step_n .le. 2501) then
!        Open(2,file = 'C:\Users\laserlab04\Desktop\case3Data\u_step' //  &
!          char(48+(step_n / 1000)) // &
!          char(48 + mod((step_n / 100),10)) // &
!          char(48 + mod((step_n / 10),10)) // &
!          char(48 + (mod(step_n,10))) // &
!           '_' // char (i_correct + 48)  // '.txt')
!	      do j=1,total_nodes
!            write(2,'(11f16.5)') u(j,step)
!	      end do
!          close(2)
!
!      endif
